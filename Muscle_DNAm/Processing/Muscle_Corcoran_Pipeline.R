...
sample_file <-read_csv(here::here("Data", "Muscle_sample_annotation_file.csv")) %>% 
...
pca.df <- pca.df[,c(ncol(pca.df),1:(ncol(pca.df)-1))]
...
summary(sapply(unique(pData(normData)$DEID), function(x) {
  barcodes <- as.character(rownames(pData(normData))[which(pData(normData)$DEID == x)])
...
# -0.27957  0.08914  1.00000  0.54874  1.00000  1.00000  

betas <- betas[grep("^cg", rownames(betas)),]

sample_file <-sample_file %>% dplyr::select(-c(OriginalOrder:ExternalSampleID, ProjectID, Plate.ID,  DNA.Concentration.1, ID.Wanagat:Female.Wanagat, -sex))


save(betas, sample_file, file=here::here("Output/Data", "Corcoran_Pipeline/CALERIE_Muscle_Corcoran_data.Rdata"))

write_csv(betas %>% as_tibble(rownames = "probe"), file=here::here("Output/Data", "Corcoran_Pipeline/CALERIE_Muscle_Corcoran_data.csv"))
write_csv(sample_file, file=here::here("Output/Data", "Corcoran_Pipeline/CALERIE_Muscle_Corcoran_sample_file.csv"))
